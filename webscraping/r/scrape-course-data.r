...
for(i in 1:num.scrapes) {
    ...
    file.connection <- file(paste(sep = "_", paste(sep="", "course-info-json-files/", program.names[i]), "full-course-info.json"))
    writeLines(prettify(toJSON(full.course.strings)), file.connection); close(file.connection)
    file.connection <- file(paste(sep = "_", paste(sep="", "course-info-json-files/", program.names[i]), "document.json"))
